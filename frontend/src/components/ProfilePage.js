    ...
    <div className="profile-page page-fade-in">
      <div className="container">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '2rem 0' }}>
          <h2>My Profile</h2>
          <button className="btn" onClick={() => setPage && setPage('dashboard')}>Back to Dashboard</button>
        </div>
        ...
        <div className="profile-card" style={{ background: 'var(--bg-white)', padding: '2rem', borderRadius: 12, boxShadow: '0 6px 18px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
            <div style={{ width: 72, height: 72, borderRadius: 12, background: 'var(--light-purple-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: 'var(--light-purple-text)' }}>AV</div>
            <div>
              <h3 style={{ marginBottom: 4 }}>Alex Volkov</h3>
              <p style={{ color: 'var(--text-medium)' }}>alex@example.com</p>
            </div>
          </div>
